...
/** Parse YYYY-MM-DD as a local calendar day (avoid `new Date("2026-03-01")` UTC → wrong month in some TZs). */
...
/** Candidate and Student each store weekOff separately; ATS calendar uses Candidate — merge so overlay matches tracking. */
function mergeWeekOffLists(lists: string[][]): string[] {
  ...
  return DAY_NAME_MAP.filter((d) => set.has(d));
}

/** When no week-off is configured, training attendance treats Sat+Sun as off (student-attendance-client isWeekOffDay). */
function effectiveWeekOffDays(mergedWeekOff: string[]): string[] {
  if (mergedWeekOff.length > 0) return mergedWeekOff;
  return ["Saturday", "Sunday"];
}

/** joiningDate / resignDate may be ISO datetime; use calendar day in local TZ. */
...
export interface StudentAttendanceOverlayProps {
  open: boolean;
  onClose: () => void;
  studentId?: string;
  userId?: string;
  /** When set, loads via GET /candidates/:id/attendance (same as ATS calendar). */
  candidateId?: string;
  studentName: string;
  initialDate?: string;
}

export default function StudentAttendanceOverlay({
  open,
  onClose,
  studentId,
  userId,
  ...
  studentName,
  initialDate,
}: StudentAttendanceOverlayProps) {
  const now = new Date();
  const [year, setYear] = useState(now.getFullYear());
  const [month, setMonth] = useState(now.getMonth());
  const [records, setRecords] = useState<AttendanceRecord[]>([]);
  const [weekOff, setWeekOff] = useState<string[]>([]);
  /** Local midnight of first day attendance applies (candidate.joiningDate ?? student.joiningDate). */
  const [joiningDateStart, setJoiningDateStart] = useState<Date | null>(null);
  /** Local midnight of last employment day (candidate.resignDate from ATS; inclusive). */
  ...
  useEffect(() => {
    ...
    const d = parseYmdLocal(initialDate);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cid) {
        try {
          const r = await getCandidateWeekOff(cid);
          parts.push(r.weekOff ?? []);
        } catch {
          parts.push([]);
        }
      }
      if (sid) {
        try {
          const r = await getStudentWeekOff(sid);
          parts.push(r.weekOff ?? []);
        } catch {
          parts.push([]);
        }
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      let jd: Date | null = null;
      let rd: Date | null = null;
      if (cid) {
        try {
          const c = await getCandidate(cid);
          jd = parseCalendarDayLocal(c.joiningDate ?? undefined);
          rd = parseCalendarDayLocal(c.resignDate ?? undefined);
        } catch {
          /* keep null */
        }
      }
      if (sid) {
        try {
          ...
          if (!jd) jd = parseCalendarDayLocal(s.joiningDate ?? undefined);
          if (!rd) {
            try {
              const list = await listCandidates({ owner: s.user.id, limit: 1, page: 1 });
              const oc = list.results?.[0];
              rd = parseCalendarDayLocal(oc?.resignDate ?? undefined);
            } catch {
              /* keep null */
            }
          }
  ...
  const fetchMonth = useCallback(async () => {
    ...
    setError(null);
    try {
      let candidateResults: AttendanceRecord[] = [];
      let studentResults: AttendanceRecord[] = [];
      let userResults: AttendanceRecord[] = [];
      ...
      if (cid) {
        try {
          const res = await listAttendanceByCandidate(cid, params);
          candidateResults = res.results ?? [];
        } catch (e: unknown) {
          candidateError =
            (e as { response?: { data?: { message?: string } } })?.response?.data?.message ??
            (e as Error)?.message ??
            "Could not load candidate attendance";
        }
      }

      if (sid) {
        try {
          const res = await listAttendance(sid, params);
          studentResults = res.results ?? [];
        } catch (e: unknown) {
          studentError =
            (e as { response?: { data?: { message?: string } } })?.response?.data?.message ??
            (e as Error)?.message ??
      ...
      if (uid) {
        try {
          const res = await listAttendanceByUser(uid, params);
          userResults = res.results ?? [];
        } catch (e: unknown) {
          userError =
            (e as { response?: { data?: { message?: string } } })?.response?.data?.message ??
            (e as Error)?.message ??
            "Could not load user attendance";
        }
      }

      const merged = [...candidateResults, ...studentResults, ...userResults].filter(
        (r, idx, arr) => arr.findIndex((x) => x.id === r.id) === idx
      );

      if (merged.length === 0) {
        const errs = [candidateError, studentError, userError].filter(Boolean) as string[];
        const tried = Boolean(cid) || Boolean(sid) || Boolean(uid);
        if (tried && errs.length > 0) {
          setError(errs[0]);
        }
      }
      setRecords(merged);
  ...
  }, [studentId, userId, candidateId, monthRange.startDate, monthRange.endDate]);

  useEffect(() => {
    if (!open || (!studentId?.trim() && !userId?.trim() && !candidateId?.trim())) return;
    fetchMonth();
  }, [open, studentId, userId, candidateId, fetchMonth]);
  ...
  const canLoadAttendance = Boolean(studentId?.trim() || userId?.trim() || candidateId?.trim());
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 id="student-attendance-title" className="text-[1.0625rem] font-semibold tracking-tight text-defaulttextcolor dark:text-white">
                  Attendance
                </h2>
                <p className="mt-0.5 text-sm text-[#64748b] dark:text-white/55 truncate">{studentName}</p>
            ...
            <button type="button" onClick={onClose} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-[#64748b] hover:bg-black/[0.04] dark:hover:bg-white/10 transition-colors" aria-label="Close">
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-4">
                {[
                  ...
                  <div key={s.label} className="rounded-xl border border-defaultborder/60 bg-white/60 dark:bg-white/[0.03] px-3 py-2.5 shadow-sm">
              ...
              {weekOff.length > 0 && (
                <div className="mb-4 flex flex-wrap items-center gap-2">
                  ...
                  {weekOff.map((d) => (
                    <span key={d} className="inline-flex items-center rounded-full border border-dashed border-teal-400/80 bg-teal-50 px-2.5 py-0.5 text-[0.75rem] font-semibold text-teal-900 dark:border-teal-500/50 dark:bg-teal-950/40 dark:text-teal-100">
                      {d}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                <h3 className="text-base font-semibold text-defaulttextcolor dark:text-white">{MONTH_NAMES[month]} {year}</h3>
              ...
              {error && <div className="mb-4 rounded-xl border border-danger/30 bg-danger/10 px-3 py-2.5 text-sm text-danger">{error}</div>}
