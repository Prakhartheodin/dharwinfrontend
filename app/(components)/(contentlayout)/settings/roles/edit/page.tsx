...
const LEGACY_DEV_TICKETS_VIEW = "devTickets.view";
...
export default function RolesEditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const originalsWithoutLegacy = originalPermissionStrings.filter(
        (p) => p.trim() !== LEGACY_DEV_TICKETS_VIEW
      );
      const permissionStrings = mergePermissionsForRoleSave(
        permissionsToApi(permissions),
        originalsWithoutLegacy
      );
              ...
              <div className="box-body">
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-6">
                    ...
                    <select
                      className="form-control form-select max-w-xs"
                      value={status}
                  ...
                  {getUnmappedPermissionStrings(originalPermissionStrings).filter(
                    (s) => s.trim() !== LEGACY_DEV_TICKETS_VIEW
                  ).length > 0 && (
                    <div className="mb-6 p-4 rounded-md border border-defaultborder bg-gray-50 dark:bg-gray-800/40">
                      ...
                      <ul className="text-xs font-mono space-y-1 list-disc list-inside text-defaulttextcolor/90">
                        {getUnmappedPermissionStrings(originalPermissionStrings)
                          .filter((s) => s.trim() !== LEGACY_DEV_TICKETS_VIEW)
                          .map((s) => (
                          <li key={s}>{s}</li>
                        ))}
                      </ul>
