...
const EditOnboarding = () => {
  ...
  useEffect(() => {
    if (!placementId) {
      setError(rawId ? 'Invalid placement ID' : null)
      setLoading(false)
      return
    }
    const load = async () => {
      ...
      try {
        ...
        const cidRaw = typeof cand === 'string'
          ? cand
          : cand && typeof cand === 'object'
            ? ((cand as { id?: string; _id?: string }).id ?? (cand as { _id?: string })._id)
            : null
    ...
    listUsers({ role: 'recruiter', limit: 200 })
      .then((r) => {
        const users = (r as { results?: { id?: string; _id?: string; name?: string }[] }).results ?? []
  ...
  return (
    <Fragment>
      <Seo title="Edit HRMS" />
      <Pageheader
        currentpage="Edit HRMS"
        activepage="Onboarding"
        mainpage={`Edit HRMS – ${candidateName}`}
      />
      <div className="grid grid-cols-12 gap-6">
