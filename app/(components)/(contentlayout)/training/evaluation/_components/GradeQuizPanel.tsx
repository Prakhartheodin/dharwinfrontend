...
import { getStudentQuizAttempts, type TrainerQuizAttemptsResponse, type TrainerQuizItem } from "@/shared/lib/api/evaluation"
...
import GradeQuizAttemptForm from "./GradeQuizAttemptForm"

export const GRADE_QUIZ_OVERLAY_ID = "grade-quiz-panel"

export interface GradeQuizPanelProps {
...
interface QuizItemBlockProps {
  item: TrainerQuizItem
  open: boolean
...
function QuizItemBlock({ item, open, openAttemptKey, formKey, onToggle, onSelectAttempt, onSaved }: QuizItemBlockProps) {
  return (
    <section className="rounded-lg border border-gray-200 dark:border-defaultborder/10 bg-white dark:bg-black/10">
      ...
      >
        ...
        <span className="text-xs text-defaulttextcolor/60 shrink-0">
          {item.pending ? "Feedback pending" : item.attempts.length ? "Reviewed" : "No attempts"}
        </span>
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-3">
          {item.attempts.length === 0 ? (
            <p className="text-sm text-gray-500">No submissions for this quiz.</p>
          ) : (
                ...
                <div key={key} className="rounded-md border border-gray-200 dark:border-white/10">
                  ...
                  {isOpen && (
                    <div className="px-3 pb-3">
                      <GradeQuizAttemptForm key={`${key}-${formKey}`} payload={attempt} onSaved={onSaved} />
                    </div>
                  )}
                </div>
              )
...
/**
 * Trainer overlay: list quiz attempts and add overall feedback.
 */
const GradeQuizPanel: React.FC<GradeQuizPanelProps> = ({
  studentId,
  studentName,
  moduleId,
  courseName,
  ...
  const [data, setData] = useState<TrainerQuizAttemptsResponse | null>(null)
  ...
  async function loadAttempts() {
    ...
    setError(null)
    try {
      const res = await getStudentQuizAttempts(studentId, moduleId)
      setData(res)
      const firstWithAttempts = res.items.find((item) => item.attempts.length)
      setOpenItemId(firstWithAttempts?.playlistItemId ?? null)
      const latest = firstWithAttempts?.attempts[0]
      setOpenAttemptKey(latest ? `${firstWithAttempts!.playlistItemId}:${latest.attemptId}` : null)
    } catch (err) {
      setError(getApiErrorMessage(err, "Could not load quiz attempts."))
      setData(null)
    } finally {
      setLoading(false)
    }
  ...
  function handleClose() {
    closeHsOverlay(`#${GRADE_QUIZ_OVERLAY_ID}`)
    onClose()
  }
        ...
        <div className="ti-offcanvas-body !p-4">
          {studentId && moduleId ? (
            <div className="space-y-4">
              ...
              {loading && <p className="text-sm text-defaulttextcolor/70">Loading quiz attempts…</p>}
              {error && (
              ...
              {emptyCourse && <p className="text-sm text-gray-500 dark:text-gray-400 py-4">No quizzes in this course.</p>}
              {nothingToGrade && <p className="text-sm text-gray-500 dark:text-gray-400 py-4">Nothing to review yet — no submitted quiz attempts.</p>}
              ...
              <div className="pt-2 border-t border-gray-200 dark:border-defaultborder/10">
                <button type="button" className={`${EVAL_BTN_LIGHT} w-full sm:w-auto`} onClick={handleClose} aria-label="Close quiz grading panel">
                  Close
                </button>
              </div>
            </div>
